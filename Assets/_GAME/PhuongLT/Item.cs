...
namespace LTPHUONG
{
    public class Item : ItemBase
    {
        ...
        [SerializeField] private Placeholder[] validPlaceholders;
        [SerializeField] private Placeholder[] requiredPlaceholders;
        [SerializeField] protected int minLayer;
        ...
        private static readonly Vector3 Squash1 = new(1.15f, 0.85f, 1f);
        private static readonly Vector3 Stretch1 = new(0.92f, 1.08f, 1f);
        private static readonly Vector3 Squash2 = new(1.05f, 0.95f, 1f);
        ...
        private void Update()
        {
            if (!isFloating || !enableFloatEffect) return;
            var pos = tf.position;
            pos.y = floatBasePosition.y + Mathf.Sin((Time.time - floatTimeOffset) / FLOAT_DURATION * TWO_PI) * floatAmplitude;
            tf.position = pos;
        }

        protected override void OnDragStart(Vector3 position)
        {
            if (isPlacingAnimation || IsCorrect) return;
            tf.DOKill();
        ...
        protected override void UpdatePosition(Vector3 targetPosition)
        {
            ...
            var target = new Vector3(targetPosition.x, targetPosition.y + dragOffsetY, targetPosition.z);
        ...
        protected virtual void PlaceCorrectly(Placeholder placeholder)
        {
            ...
            var targetPos = placeholder.TF.position;
            var midPos = (tf.position + targetPos) * 0.5f;
            midPos.y += 0.5f;
        ...
        {
            if (!IsCorrect && !isPlacingAnimation && enableFloatEffect) StartFloat();
        }

        private void StartFloat()
        {
            if (!enableFloatEffect) return;
        ...
        public override bool IsBlocked() => base.IsBlocked() || IsCorrect || isPlacingAnimation;

        private Placeholder FindClosestAvailablePlaceholder()
        {
            if (validPlaceholders == null) return null;

            Placeholder closest = null;
            float closestSqr = dropDistanceSqr;

            for (int i = 0, count = validPlaceholders.Length; i < count; i++)
            {
                var p = validPlaceholders[i];
                if (p == null || p.IsOccupied || !p.gameObject.activeInHierarchy) continue;

                float dx = tf.position.x - p.TF.position.x;
                float dy = tf.position.y - p.TF.position.y;
                float sqr = dx * dx + dy * dy;
                if (sqr < closestSqr) { closestSqr = sqr; closest = p; }
            }
            return closest;
        }
        ...
        {
            if (requiredPlaceholders == null || requiredPlaceholders.Length == 0) return true;

            for (int i = 0, count = requiredPlaceholders.Length; i < count; i++)
                if (requiredPlaceholders[i] == placeholder) return true;

            for (int i = 0, count = requiredPlaceholders.Length; i < count; i++)
                if (requiredPlaceholders[i] != null && !requiredPlaceholders[i].IsOccupied) return false;
