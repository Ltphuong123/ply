...
namespace LTPHUONG
{
    public class BroomTool : ToolBase
    {
        ...
        [SerializeField] private GameObject sweepParticle;
        ...
        protected override void OnDragging(Vector3 currentPosition)
        {
            ...
            for (int i = 1; i <= steps; i++)
                TrySweep(Vector3.Lerp(lastSweepPos, currentSweepPos, i * stepSize), speed, direction.x);
        ...
        private Vector3 GetSweepPosition() => sweepPoint != null ? sweepPoint.position : tf.position;

        private void TrySweep(Vector3 worldPos, float speed, float directionX)
        {
            if (speed < 20f || directionX >= 0) return;
            ...
            {
                var web = hitBuffer[i].GetComponent<WebItem>();
        ...
        private void PlaySweepEffect()
        {
            ...
            if (sweepParticle != null)
            {
                ...
                particleTween = DOVirtual.DelayedCall(0.5f, () => sweepParticle?.SetActive(false));
            }
            
            sweepEffectTween = DOTween.Sequence()
                .Append(tf.DORotate(originalRotation + Vector3.forward * 30f, 0.1f).SetEase(Ease.OutQuad))
                .Append(tf.DORotate(originalRotation, 0.1f).SetEase(Ease.OutQuad))
                .OnComplete(() => isPlayingSweepEffect = false);
        }
