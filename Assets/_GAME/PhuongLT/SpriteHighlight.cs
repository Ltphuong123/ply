...
namespace LTPHUONG
{
    public class SpriteHighlight : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            var mat = outlineMaterial != null
                ? new Material(outlineMaterial)
                : new Material(Shader.Find("LTPHUONG/SpriteOutlineSimple"));
        ...
        {
            KillAll();
            alphaTween = DOTween.To(() => currentAlpha, SetAlpha, 1f, showDuration)
        ...
        {
            KillAll();
            alphaTween = DOTween.To(() => currentAlpha, SetAlpha, 0f, showDuration)
                .SetEase(Ease.OutQuad);
        }
        ...
        public void Pulse()
        {
            ...
            pulseSeq = DOTween.Sequence()
                .Append(DOTween.To(() => currentAlpha, SetAlpha, 1f, pulseDuration * 0.5f).SetEase(Ease.InOutSine))
                .Append(DOTween.To(() => currentAlpha, SetAlpha, 0f, pulseDuration * 0.5f).SetEase(Ease.InOutSine))
                .SetLoops(-1, LoopType.Restart);
        }

        public void ShowInstant() { KillAll(); SetAlpha(1f); }
        public void HideInstant() { KillAll(); SetAlpha(0f); }
