...
namespace LTPHUONG
{
    ...
    {
        [SerializeField] private Collider2D[] targetZones;
        ...
        private void Update()
        {
            if (targetZones == null || targetZones.Length == 0) return;
        ...
        private void PlaySound()
        {
            ...
            if (isFadingOut)
            {
                // Đang fade out → restore volume và tiếp tục phát
                isFadingOut = false;
        ...
        private void StopSound()
        {
            ...
            isFadingOut = true;
            fadeOutTween = audioSource.DOFade(0f, 0.5f).OnComplete(() =>
            {
        ...
        private void OnDestroy() => fadeOutTween?.Kill();

        public void SetTargetZones(Collider2D[] zones) => targetZones = zones;
        public void AddTargetZone(Collider2D zone)
        {
            var list = new System.Collections.Generic.List<Collider2D>(targetZones ?? new Collider2D[0]);
