	...
	public class EmojiControl : MonoBehaviour
	{
		...
		public void HideEmoji()
		{
			if (_sequenceShowEmoji.IsActive())
			{
				...
				_sequenceShowEmoji = DOTween.Sequence().Append(scaleTransform.DOScale(Vector3.zero, durationHide / 2f).SetEase(Ease.OutQuad)).SetUpdate(true)
					.Play();
			}
		}
		...
		public void ShowPositive(float delay = 0f)
		{
			...
			_sequenceShowEmoji.Append(scaleTransform.DOScale(_originScale, durationShow).SetEase(Ease.OutBack)).AppendCallback(PlaySfx).AppendInterval(durationHold)
			...
			void PlaySfx()
			{
				if ((bool)audioSource)
				{
					audioSource.PlayOneShot(sfxPositive);
				}
			}
		}
		...
		public void ShowNegative(float delay = 0f)
		{
			...
			void PlaySfx()
			{
				if ((bool)audioSource)
				{
					audioSource.PlayOneShot(sfxNegative);
				}
			}
		}
	}
