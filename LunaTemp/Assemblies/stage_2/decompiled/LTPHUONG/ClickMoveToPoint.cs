...
namespace LTPHUONG
{
	public class ClickMoveToPoint : InteractableBase
	{
		...
		[SerializeField]
		private float squashDelay;

		[SerializeField]
		private float squashDuration = 0.36f;
		...
		[SerializeField]
		private UnityEvent onMoveBackCompleted;
		...
		private void PlaySquashStretch()
		{
			...
			Vector3 squash1 = new Vector3(baseScale.x * (1f + 0.15f * squashIntensity), baseScale.y * (1f - 0.15f * squashIntensity), baseScale.z);
			Vector3 stretch1 = new Vector3(baseScale.x * (1f - 0.08f * squashIntensity), baseScale.y * (1f + 0.08f * squashIntensity), baseScale.z);
			Vector3 squash2 = new Vector3(baseScale.x * (1f + 0.05f * squashIntensity), baseScale.y * (1f - 0.05f * squashIntensity), baseScale.z);
			float t1 = squashDuration * 0.22f;
			float t2 = squashDuration * 0.28f;
			float t3 = squashDuration * 0.22f;
			float t4 = squashDuration * 0.28f;
			DOTween.Sequence().AppendInterval(squashDelay).Append(tf.DOScale(squash1, t1).SetEase(Ease.OutQuad))
