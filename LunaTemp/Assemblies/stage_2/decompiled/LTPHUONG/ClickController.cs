...
namespace LTPHUONG
{
	public class ClickController : MonoBehaviour
	{
		...
		private void Update()
		{
			if (!(GameState.Instance != null) || GameState.Instance.IsAllowInteract)
			{
		...
		{
			int hitCount = Physics2D.RaycastNonAlloc(worldPosition, Vector2.zero, raycastHitBuffer, float.PositiveInfinity, draggableLayer);
			if (hitCount == 0)
			{
				return null;
			}
		...
		public DragBase GetCurrentDragging()
		{
