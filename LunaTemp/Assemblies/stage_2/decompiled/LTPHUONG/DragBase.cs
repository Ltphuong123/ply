...
namespace LTPHUONG
{
	public class DragBase : InteractableBase
	{
		...
		{
			Vector3 minBounds = MainCamera.ScreenToWorldPoint(new Vector3(0f, 0f, MainCamera.nearClipPlane));
			Vector3 maxBounds = MainCamera.ScreenToWorldPoint(new Vector3(Screen.width, Screen.height, MainCamera.nearClipPlane));
			return new Vector3(Mathf.Clamp(worldPosition.x, minBounds.x, maxBounds.x), Mathf.Clamp(worldPosition.y, minY, Mathf.Min(maxY, maxBounds.y)), worldPosition.z);
		}
