...
namespace LTPHUONG
{
	public class ItemContainer : InteractableBase
	{
		...
		[SerializeField]
		private float minHorizontal = 5f;

		[SerializeField]
		private float maxHorizontal = -5f;

		[SerializeField]
		private float minDistanceFromContainer = 3f;
		...
		{
			if (!isAnimating && items != null && items.Length != 0 && currentItemIndex < items.Length)
			{
				SpawnItem();
			}
		}
		...
		private void SpawnItem()
		{
			...
			PlayBounceAnimation();
			DOVirtual.DelayedCall(bounceDuration * 0.3f, delegate
			{
		...
		private void PlayBounceAnimation()
		{
			...
			Vector3 squash = new Vector3(originalScale.x * (1f + bounceIntensity), originalScale.y * (1f - bounceIntensity), originalScale.z);
			Vector3 stretch = new Vector3(originalScale.x * (1f - bounceIntensity * 0.5f), originalScale.y * (1f + bounceIntensity * 0.5f), originalScale.z);
			float t1 = bounceDuration * 0.3f;
			float t2 = bounceDuration * 0.4f;
			float t3 = bounceDuration * 0.3f;
			bounceSequence = DOTween.Sequence().Append(tf.DOScale(squash, t1).SetEase(Ease.OutQuad)).Append(tf.DOScale(stretch, t2).SetEase(Ease.OutQuad))
		...
		private void LaunchItem(Item item)
		{
			...
			do
			{
				...
				attempts++;
				distance = Vector2.Distance(new Vector2(finalPos.x, finalPos.y), new Vector2(containerPos.x, containerPos.y));
			}
			while (!(distance >= minDistanceFromContainer) && attempts < maxAttempts);
			...
			Sequence launchSeq = DOTween.Sequence().Append(item.transform.DOScale(Vector3.one, 0.2f).SetEase(Ease.OutBack)).Join(item.transform.DOMove(finalPos, launchDuration).SetEase(Ease.OutQuad))
				.OnComplete(delegate
				{
					item.Unblock();
